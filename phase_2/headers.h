...
struct Process
{
    ...
    int priority;
    int pid;
    int valid; // we may use it to check if  process that we send  is valid or it's fake process 
    int mtype; 
    int sendTime;
    int waitingTime;
    int stopTime;
...
int * shmaddr;                 // shared memory address 
//===============================
...
struct Queue
{
    //Initialize front and rear , will take value at the constructor
    node* head, * tail;
...
struct nodeWithPriority
{
    ...
    int priority; //keep in your mind that  (lower value -> high priority)
};
